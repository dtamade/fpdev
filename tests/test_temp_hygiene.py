...
class PascalTempHygieneTests(unittest.TestCase):
    ...
    def test_lazarus_config_envoptionsflow_uses_shared_temp_helpers(self):
        source_path = REPO_ROOT / 'tests' / 'test_lazarus_config_envoptionsflow.lpr'
        text = source_path.read_text(encoding='utf-8')

        self.assertIn('test_temp_paths', text)
        self.assertIn("CreateUniqueTempDir('test_lazarus_config_envoptionsflow_create')", text)
        self.assertIn("CreateUniqueTempDir('test_lazarus_config_envoptionsflow_update')", text)
        self.assertIn("CreateUniqueTempDir('test_lazarus_config_envoptionsflow_missingfile')", text)
        self.assertIn("CreateUniqueTempDir('test_lazarus_config_envoptionsflow_missingnode')", text)
