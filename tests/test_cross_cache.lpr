...
begin
  ...
  FTestsPassed := 0;
  FTestsFailed := 0;
  FTestCacheDir := GetTempDir + 'fpdev_cache_test' + PathDelim;
...
var
  SR: TSearchRec;
begin
  // Remove test cache directory and contents
  if DirectoryExists(FTestCacheDir) then
  begin
    if FindFirst(FTestCacheDir + '*', faAnyFile, SR) = 0 then
    begin
      repeat
        if (SR.Name <> '.') and (SR.Name <> '..') then
          DeleteFile(FTestCacheDir + SR.Name);
      until FindNext(SR) <> 0;
      FindClose(SR);
    end;
    RemoveDir(FTestCacheDir);
  end;
end;
