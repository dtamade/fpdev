...
> Current worktree note: later Git migrations superseded part of this audit.
> The default Git operations path now lives in `fpdev.git.operations` / `fpdev.git.operations.impl`.
> `fpdev.utils.git` compatibility shim has since been removed.
...
- No immediate change needed in this historical snapshot
...
1. **SharedGitManager → Instance Field** (Round 3, Task #11)
   ...
   - Current worktree follow-up: the default public entrypoint has since moved to `fpdev.git.operations`, and `fpdev.utils.git` no longer owns the concrete implementation
...
- Monitor any external migration fallout after removing the `fpdev.utils.git` compatibility shim
