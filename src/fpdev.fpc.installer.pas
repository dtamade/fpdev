...
type
  ...
  private
    ...
    { Generates fpc.cfg configuration file in the installation bin directory.
    ...
    { Creates Linux-specific compiler wrapper script and ppcx64 symlink.
      AInstallPath: FPC installation root
      AVersion: FPC version string }
...
function TFPCBinaryInstaller.GetFPCArchSuffix: string;
begin
  {$IFDEF CPU64}
  Result := 'x86_64';
  {$ELSE}
...
procedure TFPCBinaryInstaller.GenerateFpcConfig(const AInstallPath, AVersion: string);
var
...
begin
  ...
  try
    ...
    SaveToFile(AInstallPath + PathDelim + 'bin' + PathDelim + 'fpc.cfg');
...
procedure TFPCBinaryInstaller.CreateLinuxCompilerWrapper(const AInstallPath, AVersion: string);
begin
  {$IFDEF LINUX}
  ...
  TProcessExecutor.Execute('ln', ['-sf',
    AInstallPath + PathDelim + 'lib' + PathDelim + 'fpc' + PathDelim + AVersion + PathDelim + 'ppcx64',
    AInstallPath + PathDelim + 'bin' + PathDelim + 'ppcx64'], '');
  ...
  try
    ...
    Add(AInstallPath + '/bin/ppcx64 -n @' + AInstallPath + '/bin/fpc.cfg "$@"');
    SaveToFile(AInstallPath + PathDelim + 'bin' + PathDelim + 'fpc.sh');
  finally
    Free;
  end;
  TProcessExecutor.Execute('chmod', ['+x', AInstallPath + PathDelim + 'bin' + PathDelim + 'fpc.sh'], '');
  // Replace the fpc binary with our wrapper
  TProcessExecutor.Execute('mv', [AInstallPath + PathDelim + 'bin' + PathDelim + 'fpc',
    AInstallPath + PathDelim + 'bin' + PathDelim + 'fpc.orig'], '');
  TProcessExecutor.Execute('mv', [AInstallPath + PathDelim + 'bin' + PathDelim + 'fpc.sh',
...
begin
  ...
  try
    ...
    begin
      CreateLinuxCompilerWrapper(InstallPath, AVersion);
      GenerateFpcConfig(InstallPath, AVersion);
    end;
