...
var
  ...
  I: Integer;
  LFPCPath: string;
  LCfgPath, LLibPath, LCacheDir: string;
  LDiskFree: Int64;
begin
  Result := 0;
  LIssueCount := 0;
  ...
  // 8) fpc.cfg validation for installed toolchains
  ...
  begin
    LOk := CheckWriteableDir(LCacheDir, LErr);
    if LOk then
  ...
  LRoot := GetToolchainsDir;
  try
    LDiskFree := DiskFree(0);  // 0 = current drive
    if LDiskFree > 0 then
    begin
      if LDiskFree < 500 * 1024 * 1024 then  // < 500 MB
      begin
        Ctx.Out.WriteLn('  WARNING: Low disk space (' + IntToStr(LDiskFree div (1024*1024)) + ' MB free)');
        Ctx.Out.WriteLn('    Hint: FPC builds require at least 500 MB free space');
        Inc(LIssueCount);
      end
      else
        Ctx.Out.WriteLn('  OK: ' + IntToStr(LDiskFree div (1024*1024)) + ' MB free disk space');
    end
    else
