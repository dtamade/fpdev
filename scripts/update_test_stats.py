...
def render_release_notes_md(text: str, count: int) -> str:
    return replace_once(
        text,
        r'^Test inventory: \d+ discoverable test_\*\.lpr programs(?: \(same inventory rules as CI\))?$',
        f'Test inventory: {count} discoverable test_*.lpr programs (same inventory rules as CI)',
        RELEASE_NOTES_MD,
    )
